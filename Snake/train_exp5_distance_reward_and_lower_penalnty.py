...
class AdvancedSnakeEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        obs = []
        directions = [Vector2(0, -1), Vector2(1, -1), Vector2(1, 0), Vector2(1, 1), Vector2(0, 1), Vector2(-1, 1), Vector2(-1, 0), Vector2(-1, -1)]
    ...
    def step(self, action):
        ...
        if head == self.fruit_pos:
            ...
            # The rewards for guiding the AI from the first experiment return and the penalnty per frame becomes less
                reward = 0.1
            else:
                reward = -0.1
            reward -= 0.005
...
if __name__ == "__main__":
    EXP_NAME = "Exp_5_Distance_Reward_and_Lower_Penalnty"
    ...
    # The AI is given a different ent_coef from the fourth experiment
    # The gamma from the fourth experiment has been removed
    policy_kwargs = dict(net_arch=dict(pi=[256, 256], vf=[256, 256]))
    model = PPO("MlpPolicy", train_env, verbose=1, policy_kwargs=policy_kwargs, ent_coef=0.02)
